...
namespace Cubequad.Tentacles2D
{
    ...
    public class Tentacle : MonoBehaviour
    {
        ...
        private Vector2[] uvs;
        private Vector2 storedDirection; //normalized
        private Mesh mesh;
        Material myMaterial;
        ...
        private Vector2 coordsInDread;
        private MaterialPropertyBlock materialBlock;

        [SerializeField] private int jointsCount = 2;
        ...
        public Vector2 CoordsInDread { get => coordsInDread; set { coordsInDread = value; } }

        public Vector2 StoredDirection { get => storedDirection; set { storedDirection = value; } }
        /// <summary>
        ...
        [SerializeField] [Range(1, 25)] private int tentacleReach;
        [SerializeField] private float frequency = 1f;
        [SerializeField] private float amplitude = 1f;
        [SerializeField] private float animationDelay = 0f;
        private float opacityMultiplier = 7;
        ...
        public Rigidbody2D[] auxSegments { get; private set; }
        /// <summary>
        ...
        public float SetLength { get; set; }
        ...
        private void Awake()
        {
            ...
            transform.position += new Vector3(0,0, UnityEngine.Random.Range(0.0f, -2.5f));
            Initialize();
            InitializeMesh();
            InitializeUVs();
        }
        ...
        private void FixedUpdate() // https://docs.unity3d.com/ScriptReference/MonoBehaviour.FixedUpdate.html (basically for physics stuff that should run independed of framerate (default is 50 calls per second))
        {
            if (pivotHingeJoint.connectedBody != null)
            {
        ...
        private void Initialize()
        {
            ...
            Segments = new Rigidbody2D[jointsCount];
            for (int i = 0; i < Segments.Length;)
                Segments[i++] = transform.GetChild(i).GetComponent<Rigidbody2D>();
            Tip = transform.GetChild(Segments.Length + 1).GetComponent<Rigidbody2D>();
        ...
        private void DefineVertices()
        {
            ...
            for (int i = 1; i < smoothness; i++)
            {
                ...
                //Vector2 dist = points[i - 1] - points[i];
                //vertices[i + firstIndex] = points[i] + new Vector2(dist.y, -dist.x).normalized * curve * width;
                //vertices[lastIndex - i] = points[i] + new Vector2(dist.y * -1f, -dist.x * -1f).normalized * curve * width;
        ...
        private void RenderMesh()
        {
         ...
         Color[] colors = new Color[verticesLength];
            ...
            // float opacityMultiplier = 7f;
            if(fade)
            {
                opacityMultiplier *= 0.9f;
                if (opacityMultiplier < 0.1f)
                {
                   Destroy(gameObject);
                }
            }
            int intensityPower = 3;
            float halfLength = verticesLength / 2;
            Color myColor = new Color(1, 1, 1, 1f);
            for (int i = 0; i < halfLength; i++)
            {
                myColor.a = Mathf.Min(Mathf.Pow((float)i / halfLength, intensityPower) * opacityMultiplier, 1f);
                colors[i] = myColor;
            }
            for (int i = (int)halfLength; i < verticesLength; i++)
            {
                myColor.a = Mathf.Min(Mathf.Pow((float)(verticesLength - i) / halfLength, intensityPower) * opacityMultiplier, 1f);
                colors[i] = myColor;
            }
            ...
            //{
            //    Debug.Log(meshFilter.sharedMesh.colors[i] + " " + i);
            //}
        ...
        {
            if(tipHingeJoint.connectedBody == null)
            {
                if (target != null)
                { // calc distance to target, increase length of tentacle every frame there is a target ( make sure pulling force(speed) is strong enaugh), when target is released (make target a point, not pos of qin), gradualy decrease tentacle length to original state.
                  //var direction = (Vector2)target.position - Tip.position;
                    ...
                    Length += Mathf.Sqrt(Vector2.Distance((Vector2)Pivot.position, Tip.position)) * Time.deltaTime * tentacleReach;
                    Tip.AddForce(((Vector2)target.position - Tip.position) * Time.deltaTime * speed / multiplier); //1000
                    // Length += Vector2.Distance((Vector2)target.position, Tip.position) / (30 - tentacleReach) / multiplier; // 30 max reach
                    // Length -= Vector2.Distance((Vector2)Pivot.position, Tip.position) / (30 - tentacleReach);
                    Debug.DrawLine(target.position, Tip.position, Color.gray); //#rememberMe
                }
                else if (Length > SetLength) // * the scale of the base's parent (0.5 on manual), which is Dread
                {
                    Length -= Mathf.Sqrt(Vector2.Distance((Vector2)Pivot.position, Tip.position)) * Time.deltaTime * tentacleReach;
                    if (Length < SetLength)
                    {
        ...
        /// </summary>
        public void Catch() // instantly touch target
        {
        ...
        public IEnumerable AttackTimer()
        {
            IsAttacking = true;
        ...
        private struct Utilities
        {
            ...
            public static Vector2 GetPointFromSegmentArr(Rigidbody2D[] segmentArr, float t)
            {
                float u = 1 - t;
                ...
                for (int i = 0; i < segmentArr.Length; i++)
                {
                    if(i == 1 || i == 2)
                    {
                        sum += 3 * Mathf.Pow(u, segmentArr.Length - 1 - i) * Mathf.Pow(t, i) * segmentArr[i].position;
                    } else
                    {
                        sum += Mathf.Pow(u, segmentArr.Length - 1 - i) * Mathf.Pow(t, i) * segmentArr[i].position;
                    }
